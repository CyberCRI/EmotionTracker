    ...
    snapping: function() {
      ...
      return $(window).off("scroll.snapscroll").on("scroll.snapscroll", function() {
        ...
        if (!autoscrolling) {
          ...
          if ($child) {
            if(iframeContent.isRecord){
              var tempValue = iframeContent.stopRecording();
              iframeContent.postResult(tempValue,iframeContent.actualPageID);
              iframeContent.startRecording($child.index());
            }
            clearTimeout(timer);
